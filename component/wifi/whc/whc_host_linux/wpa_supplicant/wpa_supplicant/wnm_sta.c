...
void wpa_ft_start_over_ds_hdl(void *eloop_ctx, void *timeout_ctx){
	...
	if(timeout_ctx == NULL){
		wpa_printf(MSG_ERROR, "FT DS: invalid "
			   "address");
		return;
	}
	memcpy(target_ap, timeout_ctx, ETH_ALEN);
	wpa_hexdump(MSG_DEBUG,"WNM address", (u8*)timeout_ctx, ETH_ALEN);

	wpa_printf(MSG_DEBUG, "BTM triggers FT_DS " MACSTR, MAC2STR(target_ap));
	...
	if(wpa_ft_start_over_ds(wpa_s->wpa, target_ap, mdie))
	 	wpa_printf(MSG_ERROR, "ft start over ds process fail\n");
}
			       ...
			       int after_new_scan)
{
	...
	wpa_printf(MSG_DEBUG, "WNM: mdie_ft_capab=%x", wpa_get_ft_mdie_capab(sm));
	if(wpa_get_ft_mdie_capab(sm) & BIT(0)){  /**/
		wpa_printf(MSG_DEBUG, "WNM: Over-the-DS case. Call function start ft ds\n");
		wpa_hexdump(MSG_DEBUG, "WNM address",
				    bss->bssid, 6);
		eloop_cancel_timeout(wpa_ft_start_over_ds_hdl, wpa_s, bss->bssid);
		eloop_register_timeout(0, 200, wpa_ft_start_over_ds_hdl, wpa_s, bss->bssid);
	}
	else{
