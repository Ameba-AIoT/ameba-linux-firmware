...
exit:
	...
	if (pairwise && global_idev.is_need_4way[wlan_idx] && global_idev.is_4way_ongoing[wlan_idx]) {
		if (1 == wlan_idx && (--global_idev.is_4way_ongoing[wlan_idx])) {//softap mode, if no client ongoing 4-way process, notify NP end. otherwise, not notify 4-way end.
			return ret;
		}
		...
		dev_dbg(global_idev.fullmac_dev, "[fullmac]: llhw notify 4-way end");
