...
static int cfg80211_rtw_del_station(struct wiphy *wiphy, struct net_device *ndev, struct station_del_parameters *params)
{
	...
	dev_dbg(global_idev.fullmac_dev, "[fullmac]: wlan_idx = %d, is_need_4way= %d, is_4way_ongoing =%d", wlan_idx, global_idev.is_need_4way[wlan_idx], global_idev.is_4way_ongoing[wlan_idx]);
	...
	if (global_idev.is_need_4way[wlan_idx] && global_idev.is_4way_ongoing[wlan_idx] > 0) {
		...
		if (0 == global_idev.is_4way_ongoing[wlan_idx]) {// no client ongoing 4-way process, then notify NP end. otherwise, not notify 4-way end.
			...
			ret = llhw_wifi_wpa_4way_status_indicate(&rpt_4way);
...
void cfg80211_rtw_sta_assoc_indicate(char *buf, int buf_len)
{
	...
	dev_dbg(global_idev.fullmac_dev, "[fullmac]: wlan_idx = %d, is_need_4wway= %d, is_4way_ongoing =%d", wlan_idx, global_idev.is_need_4way[wlan_idx], global_idev.is_4way_ongoing[wlan_idx]);
...
#else
		llhw_wifi_wpa_4way_status_indicate(&rpt_4way);
#endif
		global_idev.is_4way_ongoing[wlan_idx]++;
		dev_dbg(global_idev.fullmac_dev, "llhw indicate 4-way start, ongoing 4way client cnt=%d \n", global_idev.is_4way_ongoing[wlan_idx]);
	}
}
