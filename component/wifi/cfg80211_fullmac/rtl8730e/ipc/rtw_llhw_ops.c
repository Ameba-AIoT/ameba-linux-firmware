...
int llhw_wifi_wpa_4way_status_indicate(struct rtw_wpa_4way_status *rpt_4way)
{
	int ret = 0;
	u32 param_buf[1];
	dma_addr_t dma_addr_4way;
	struct device *pdev = global_idev.ipc_dev;
	struct rtw_wpa_4way_status *prpt_4way;

	prpt_4way = kmalloc(sizeof(struct rtw_wpa_4way_status), GFP_KERNEL);
	if (!prpt_4way) {
		dev_err(global_idev.fullmac_dev, "%s: malloc failed!\n", __func__);
		return -ENOMEM;
	}
	...
	dma_addr_4way = dma_map_single(pdev, prpt_4way, sizeof(struct rtw_wpa_4way_status), DMA_TO_DEVICE);
	if (dma_mapping_error(pdev, dma_addr_4way)) {
		dev_err(global_idev.fullmac_dev, "%s: mapping dma error!\n", __func__);
		return -1;
	}

	param_buf[0] = (u32)dma_addr_4way;
	ret = llhw_ipc_send_msg(INIC_API_WPA_4WAY_REPORT, param_buf, 1);

	dma_unmap_single(pdev, dma_addr_4way, sizeof(struct rtw_wpa_4way_status), DMA_TO_DEVICE);
	kfree(prpt_4way);
	return ret;
}
