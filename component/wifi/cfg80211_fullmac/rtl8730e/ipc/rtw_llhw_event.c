	...
	if (event == WIFI_EVENT_STA_DISASSOC) {
		...
		if (global_idev.is_need_4way[wlan_idx] && global_idev.is_4way_ongoing[wlan_idx] > 0) {
			...
			if (0 == global_idev.is_4way_ongoing[wlan_idx]) {// no client ongoing 4-way process, then notify NP end. otherwise, not notify 4-way end.
				...
				llhw_wifi_wpa_4way_status_indicate_work(&rpt_4way);
			}
		}
...
{
	struct device *pdev = NULL;
	struct rx_pkt_info *ppktinfo = (struct rx_pkt_info *)llhw_ipc_fw_phy_to_virt(p_ipc_msg->param_buf[0]);
...
static void llhw_event_nan_cfgvendor_cmd_reply(struct event_priv_t *event_priv, struct inic_ipc_dev_req_msg *p_ipc_msg)
{
	...
	unsigned char *data_addr = llhw_ipc_fw_phy_to_virt(param_buf[1]);
...
static void llhw_api_coex_notify_work_hdl(struct work_struct *data)
{
	struct api_coex_notify_work_t *wd = container_of(data, struct api_coex_notify_work_t, work);

	llhw_wifi_wpa_4way_status_indicate(&wd->data);
	dev_dbg(global_idev.fullmac_dev, "llhw indicate port %d 4-way %s\n",
			wd->data.wlan_idx, wd->data.is_start ? "start" : "end");

	kfree(wd);
}

void llhw_wifi_wpa_4way_status_indicate_work(struct rtw_wpa_4way_status *rpt_4way)
{
	struct api_coex_notify_work_t *wd = kmalloc(sizeof(*wd), GFP_KERNEL);

	if (wd == NULL) {
		dev_err(global_idev.fullmac_dev, "%s: allloc api_coex_notify_work_t error.\n", "event");
		return;
	}

	memcpy(&wd->data, rpt_4way, sizeof(struct rtw_wpa_4way_status));
	INIT_WORK(&wd->work, llhw_api_coex_notify_work_hdl);
