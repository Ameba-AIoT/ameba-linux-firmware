...
struct api_coex_notify_work_t {
	struct work_struct work; /* to avoid bloack of tasklet, add this work to send ipc msg to device */
	struct rtw_wpa_4way_status data;
};
...
struct inic_device {
	...
	u8				is_need_4way[INIC_MAX_NET_PORT_NUM]; /* 0: no need, 1: need */
	u8				is_4way_ongoing[INIC_MAX_NET_PORT_NUM]; /* 0: 4-way is not ongoing, 1: 4-way is ongoing */
	/* wifi user config */
